...
def run_eval(args):
    ...
    if args.eval_mode == 'stats':
        auc=evaluate(ckpt_args.dataset_split, model, dl, ckpt_args)
        
        #if args.method!='fc':
          #logpns= evaluate_for_pns(ckpt_args.dataset_split, model, dl, ckpt_args)
          #print(f'logpns average:{logpns}')
          
          #print(f'counts of violations average:{violation}')
          #return acc,logpns
        return auc
